...
if pipeline_names:
    ...
    pipeline_artifacts = [
        artifact for artifact in all_artifacts if f'pipeline:{selected_pipeline_name}' in artifact.tags
    ]

    st.write(f"## Pipeline: {selected_pipeline_name}")

    model_artifact = next((artifact for artifact in pipeline_artifacts if artifact.type == 'model'), None)
    if model_artifact:
    ...
    if st.button("Delete Pipeline"):
        st.session_state['delete_confirmation'] = selected_pipeline_name

    if st.session_state.get('delete_confirmation') == selected_pipeline_name:
        st.warning(f"Are you sure you want to delete the pipeline '{selected_pipeline_name}'?")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Yes, delete it"):
                for artifact in pipeline_artifacts:
                    automl.registry.delete(artifact.id)
                st.success(f"Pipeline '{selected_pipeline_name}' deleted successfully.")
        ...
        if model_type == 'classification':
            st.write("This is a **classification** pipeline. Please upload data appropriate for classification tasks.")
        elif model_type == 'regression':
            st.write("This is a **regression** pipeline. Please upload data appropriate for regression tasks.")
        else:
        ...
        if uploaded_file is not None:
            ...
            pipeline_config_artifact = next((artifact for artifact in pipeline_artifacts if artifact.name.endswith("pipeline_config")), None)
            if pipeline_config_artifact:
                ...
                preprocessing_artifacts = {
                    artifact.name: artifact for artifact in pipeline_artifacts if artifact.type == 'preprocessing'
                }
                ...
                for feature in input_features:
                    ...
                    try:
                        feature_data = new_data_df[feature_name].values.reshape(-1, 1)
                    except KeyError:
                        st.error(f"Change the uploaded dataset type to the correct one, which should be a {model_type} dataset.")
                        st.stop()
                    artifact_name = f"{selected_pipeline_name}_{feature_name}"
                    if artifact_name in preprocessing_artifacts:
                        preprocessor_artifact = preprocessing_artifacts[artifact_name]
